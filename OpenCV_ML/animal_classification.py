...
#helper function to convert a sample and classification into a pair of Numpy arrays
...
for e in range (0, epochs):
    ...
    for t,c in records:
        ...
        if animal_net.isTrained():
            animal_net.train(data, cv.ml.ANN_MLP_UPDATE_WEIGHTS |
            cv.ml.ANN_MLP_NO_INPUT_SCALE | cv.ml.ANN_MLP_NO_OUTPUT_SCALE) 
            #update weights allows us pass previous weights to new model threfore, 
            # we do not start traing the model from scratch -> training a model incrementally
        else:
            animal_net.train(data, cv.ml.ANN_MLP_NO_INPUT_SCALE | 
